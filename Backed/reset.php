 ...
 if(isset($_GET['action']))
{
    if($_GET['action']=="reset")
    {
		...
		if(isset($_GET['u']) && $_GET['u']=='c'){
			$query = "SELECT r_id as id FROM ".REGISTER." where md5(1290*3+r_id)='".$encrypt."'";
			$catfet=$prop->getAll_Disp($query);
			$opt = 'company';
		}else{
...
if(isset($_POST['resetSubmit']))
{
	...
	if(count($result)>0)
	{
		...
		if($prop->update($table, $insdata, $t_cond))
		{
        ...
        <div class="login-box">
            <div class="white-box">
                ...
                <form class="form-horizontal form-material" id="loginform" method="post">
					<input type="hidden" name="option" value="<?php echo $opt; ?>"/>
