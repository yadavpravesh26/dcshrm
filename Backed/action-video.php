...
switch($method)
{
	case 'add':
        ...
        if ($res) {
			...
			setcookie("title", "Video Created Successfully", time()+10);
			...
			header('Location: action-video.php');
		}
	break;
	case 'update':
		...
		{
			setcookie("status", "Success", time()+10);
			setcookie("title", "Video Updated Successfully", time()+10);
			setcookie("err", "success", time()+10);
			header('Location: manage-video.php');
		}else{
			setcookie("status", "Error", time()+10);
            setcookie("title", $msg, time()+10);
            setcookie("err", "error", time()+10);
			header('Location: action-video.php?method=modify&id='.$_REQUEST['id']);
		}
...
if(isset($_REQUEST['id']) && $_REQUEST['id']!=''){
	$titleTag = 'Edit';
	$curr_val = $prop->get('*',$table_name, array("doc_id"=>$_REQUEST['id'],'doc_type'=>3));
	if(empty($curr_val)){
		header('Location: manage-video.php');
		exit;
	}
...
<head>
	...
	<title><?php echo $titleTag; ?> Video</title>
	<!-- Bootstrap Core CSS -->
			...
			<div class="container-fluid">
				<div class="row bg-title">
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
						<h4 class="page-title"><?php echo $titleTag; ?> Video</h4> </div>
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
						<ol class="breadcrumb">
							<li><a href="dashboard.php">Dashboard</a>
							</li>
							<li class="active"><?php echo $titleTag; ?> Video</li>
						</ol>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<div class="row">
					<div class="col-md-12">
						<div class="white-box">
							<h3 class="box-title"><?php echo $titleTag; ?> Video</h3>

							<?php $foraction = (isset($_REQUEST['id'])?'update&id='.$_REQUEST['id']:'add');?>

							<form data-toggle="validator" method="post" action="action-video.php?method=<?php echo $foraction; ?>" enctype="multipart/form-data" id='categoryform'>
								<div class="row">
									...
									<div class="form-group col-md-6">
										<label for="exampleInputuname1">Youtube Link</label>

										<div class="input-group">

											<input type="text" class="form-control" value="<?php echo $curr_val['doc_file'];?>" name="video" id="exampleInputuname1" placeholder="Ex: https://www.youtube.com/watch?v=00f0A1ppnRo" required>
										</div>
										<div class="help-block with-errors"></div>
									</div>
									...
									<div class="clearfix"></div>
									<div class="col-sm-12">
										<a href="manage-video.php"><button type="button"  class="btn btn-inverse waves-effect waves-light pull-right">Cancel</button></a>
