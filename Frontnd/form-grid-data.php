...
for($i=0; $i<count($row); $i++)
	...
	$eq.='<a href="pdf/form_pdf_print.php?id='.$row[$i]['form_id'].'&row_id='.$row[$i]['id'].'&type=D&method=frm" target="_blank" class="btn-rounded btn-success btn-outline v-btn" data-toggle="tooltip" data-original-title="Download"> <i class="icon-cloud-download"></i> </a>';
	if($row[$i]['reassign'])
	$eq.='<a href="form-page.php?id='.$row[$i]['form_id'].'" class="btn-rounded btn-warning btn-outline m-r-10 v-btn" data-toggle="tooltip" data-original-title="Edit"><i class="ti-reload"></i></a>';
	
	//$eq.='<button class="btn-rounded btn-danger btn-outline v-btn" data-toggle="tooltip" data-original-title="Delete" onclick="deleteform('.$row[$i]['id'].')"> <i class="i-font17 ti-trash"></i> </button>';
	 $nestedData[] = $eq;
	$data[] = $nestedData;
}
