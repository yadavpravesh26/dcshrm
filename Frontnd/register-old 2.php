...
require('authnetXML/config.inc.php');
	...
	if(count($post)>15){
		...
		if($count===0){
			...
			if($xml->isSuccessful()){
				...
				$customerShipAddId = $xml->customerShippingAddressIdList->numericString;
				...
				if ($result>0) {
					...
					$xml2 = new AuthnetXML(AUTHNET_LOGIN, AUTHNET_TRANSKEY, AuthnetXML::USE_DEVELOPMENT_SERVER);
					$xml2->createCustomerProfileTransactionRequest(array(
						'transaction' => array(
						...
						'extraOptions' => '<![CDATA[x_customer_ip='.$ipAdd.']]>'
					));
					
					if($xml2->isSuccessful()){
						$f_msg = $desc.' / Invoice Number : '.$invoiceNumber.' Please call orriant at 888-346-0990 for further details /';
						$exp = $xml2->directResponse;
						$arr = explode(",",$exp);
						$card_number = $arr[50];
						$brand = $arr[51];
						$transaction_no = $arr[6];
						$insert = array(
						'reg_id'=>$result,
						'plan'=>$plan,
						'amount'=>$amount,
						...
						$insert['merchantCustomerId'] = $merchantCustomerId;
						$insert['customerProfileId'] = $customerProfileId;
						$insert['customerPaymentProfileId'] = $customerPaymentProfileId;
						...
						if($res>0){
							...
							$o_out = $prop->update(USERS, $append_post, array('id'=>$result));
							$msg = $f_msg;
						}
						/* Message */
						$alert['status'] = 'Success';
						$alert['err'] = 'success';
						$alert['title'] = $msg;
						$xml3 = new AuthnetXML(AUTHNET_LOGIN, AUTHNET_TRANSKEY, AuthnetXML::USE_DEVELOPMENT_SERVER);
						$xml3->ARBCreateSubscriptionRequest(array(
							'refId' => time(),
							'subscription' => array(
								'name' => substr($post['email'],0,30),
						...
						if($xml3->isSuccessful()){
							$subscriptionId = $xml3->subscriptionId;
							$prop->update(USERS, array("subscriptionId"=>$subscriptionId),array('id'=>$result));
							$prop->update(PAYMENT, array("subscriptionId"=>$subscriptionId),array('id'=>$res));
							$to=$post['email'];
						...
						if($xml3->isError()){
							$msg = $xml3->messages->message->text;
							$msg = $f_msg.$msg;
							/* Message */
							$alert['status'] = 'Error';
							$alert['err'] = 'error';
					...
					if($xml2->isError()){
						$msg = $xml2->messages->message->text;
						$prop->update(USERS, array("status"=>2),array('id'=>$result));
						/* Message */
						$alert['status'] = 'Error';
						$alert['err'] = 'error';
						$alert['title'] = $msg;
			...
			$alert['status'] = 'Error';
			$alert['err'] = 'error';
			$alert['title'] = 'Email Address Already Available';
		}
	}else{
		/* Message */
		$alert['status'] = 'Error';
		$alert['err'] = 'error';
		$alert['title'] = 'Please fill all mandatory fields';
	}
	setcookie("status", $alert['status'], time()+10);
	setcookie("title", $alert['title'], time()+10);
	setcookie("err", $alert['err'], time()+10);
